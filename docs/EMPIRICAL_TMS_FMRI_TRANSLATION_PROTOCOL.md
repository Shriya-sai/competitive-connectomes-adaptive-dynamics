...
#### Multisite participant gate

The same frozen preprocessing, AR(1) event model, cortical-coverage rules and
spatial-response instrument were subsequently applied to all 11 stimulation
sites for NTHC1035. All designs were full rank and produced finite maps. Ten
runs passed the descriptive motion gate of no more than 20% censored volumes;
R-IPL was flagged because 44/164 volumes (26.8%) exceeded the 0.5 mm FD rule.

No site showed a detectable local mean response when conservatively applying
the same absolute-z threshold of 3.1 used for remote extent, and no eligible
remote parcel crossed that frozen threshold at any
site. The largest absolute local mean z was 0.77 (R-M1), and the largest
absolute remote parcel-mean z was 1.30 (also R-M1). The participant gate
therefore fails: NTHC1035 will not be carried into the resting-embedding
analysis, because there is no reliably detected perturbational response for
the embedding to explain. This is a failure of the proposed empirical bridge
in this pilot dataset, not evidence against cooperative/competitive network
dynamics or against a biological TMS effect. The complete machine-readable
classification is generated by `scripts/summarize_tms_multisite_gate.py`.
